...
lemma preimagen_interseccion_familia'_solucion (X Y : Type) (f : X → Y) (F : set (set Y)) : f ⁻¹' (⋂₀ F ) = ⋂₀ { (f ⁻¹' V) | V ∈ F} :=
begin
  ext x,
  split,
  {
...
lemma preimagen_union_familia'_solucion (X Y : Type) (f : X → Y) (F : set (set Y)) : f ⁻¹' (⋃₀ F ) = ⋃₀ {(f ⁻¹' V) | V ∈  F} :=
begin
  ext x,
  split,
  {
...
example (X Y : Type) (f : X → Y) (V : set Y) : f '' (f ⁻¹' V ) ⊆ V :=
begin
  intros y hy,
  cases hy with x hx,
  cases hx with hxf hxy,
...
example (X Y : Type) (f : X → Y) (U : set X) :
U ⊆ f ⁻¹' (f '' U) :=
begin
  intros x hx,
  use x,
